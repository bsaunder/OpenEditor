 ...
 * have a TextFinder which supports the GURL event, which is true for Mac OS 8.0 and 8.1 systems that
...
public class BrowserLauncher {
	...
	/**
	 * The file type of the TextFinder on a Macintosh.  Hardcoding "TextFinder" would keep non-U.S. English
	 * systems from working properly.
	 */
	private static final String FINDER_TYPE = "FNDR";

	/**
	 * The creator code of the TextFinder on a Macintosh, which is needed to send AppleEvents to the
	 * application.
	 */
			...
			case MRJ_2_1:
				...
				for(int i = 0; i < systemFolderFiles.length; i++) {
					try {
						...
						// a type of 'FNDR'.  Only requiring the type results in non-TextFinder
						...
						if (FINDER_TYPE.equals(fileType.toString())) {
							...
							if (FINDER_CREATOR.equals(fileCreator.toString())) {
								browser = file.toString();	// Actually the TextFinder, but that's OK
								return browser;
							}
						}
					} catch (IllegalArgumentException iare) {
